...
TextureGL BVHBuilder::bvhToTexture()
{
	...
	return TextureGL(TextureGLType::TextureBufferXYZ, nodeList.size() * sizeof(Node), nodeList.data());
}
...
void BVHBuilder::buildRecurcive(int nodeIndex, std::vector<Triangle> const& vecTriangle)
{
	...
	if (axis == 0)
        splitByAxis(std::bind(&vec3::x, _1));

	if (axis == 1)
        splitByAxis(std::bind(&vec3::y, _1));

	if (axis == 2)
        splitByAxis(std::bind(&vec3::z, _1));
