#version 330 core

in vec2 fragCoord;
out vec4 color;

uniform vec3 location;
uniform float aspectRation;
uniform mat3 viewToWorld;
uniform samplerBuffer texPosition;
uniform samplerBuffer texNode;
uniform samplerBuffer texPreInData;
uniform isamplerBuffer texIndex;

//------------------- STRUCT AND LOADER BEGIN -----------------------
struct Triangle
{
    // Position
    vec3 pos1;
    vec3 pos2;
    vec3 pos3;
};
struct Node
{
    int childIsTriangle;
    int leftChild; // or count
    int rightChild; // or startIndex
    vec3 aabbMin;
    vec3 aabbMax;
};

struct Ray
{
    vec3 origin;
    vec3 direction;
    float tStart;
    float tEnd;
};

struct Hit
{
    vec3 normal;
    vec3 position;
    vec2 uv;
    bool isHit;
};

Node getNode(int index)
{
	index = index * 3;

	vec3 integerData = texelFetch(texNode, index).rgb;
	vec3 aabbMin = texelFetch(texNode, index + 1).rgb;
	vec3 aabbMax = texelFetch(texNode, index + 2).rgb;

	Node node;
	node.childIsTriangle = floatBitsToInt(integerData.x);
	node.leftChild = floatBitsToInt(integerData.y);
	node.rightChild = floatBitsToInt(integerData.z);
	node.aabbMin = aabbMin;
	node.aabbMax = aabbMax;
	return node;
}

Triangle getTriangle(int index)
{
	index = index * 3;
	Triangle triangle;
	triangle.pos1 = texelFetch(texPosition, index).rgb;
	triangle.pos2 = texelFetch(texPosition, index + 1).rgb;
	triangle.pos3 = texelFetch(texPosition, index + 2).rgb;
	return triangle;
}
//------------------- STRUCT AND LOADER END -----------------------

//------------------- STACK BEGIN -----------------------
int countTI = 0;
int _stack[15];
int _index = -1;

void stackClear()
{
    _index = -1;
}

int stackSize()
{
    return _index + 1;
}

void stackPush(in int node)
{
    if(_index > 14	)
        discard;
    _stack[++_index] = node;
 
}

int stackPop()
{
    return _stack[_index--];
}
//------------------- STACK END -----------------------

bool slabs(in Ray ray, in vec3 minB, in vec3 maxB, inout float localMin) {

    if(all(greaterThan(ray.origin, minB)) && all(lessThan(ray.origin, maxB)))
        return true;

    vec3 t0 = (minB - ray.origin)/ray.direction;
    vec3 t1 = (maxB - ray.origin)/ray.direction;
    vec3 tmin = min(t0, t1), tmax = max(t0, t1);
    float tminf = max(max(tmin.x, tmin.y), tmin.z);
    float tmaxf = min(min(tmax.x, tmax.y), tmax.z);

    if (tminf > tmaxf)
        return false;

    localMin = tminf;
    return tminf < ray.tEnd && tminf > ray.tStart;
}

bool isect_tri(inout Ray ray, in Triangle tri, inout Hit hit ) {
	vec3 e1 = tri.pos2 - tri.pos1;
	vec3 e2 = tri.pos3 - tri.pos1;
	vec3 P = cross(ray.direction, e2);
	float det = dot(e1, P);
	if (abs(det) < 1e-4)
        return false;

	float inv_det = 1. / det;
	vec3 T = (ray.origin - tri.pos1);
	float u = dot(T, P) * inv_det;
	if (u < 0.0 || u > 1.0)
        return false;

	vec3 Q = cross(T, e1);
	float v = dot(ray.direction, Q) * inv_det;
	if (v < 0.0 || (v+u) > 1.0)
        return false;

	float tt = dot(e2, Q) * inv_det;

    if(ray.tEnd > tt && ray.tStart < tt )
    {
        vec3 c = vec3(u, v, 1.0 - u - v);
        countTI++;
        hit.normal = normalize(cross(e1, e2));//(tri.norm1 * c.z + tri.norm2 * c.x + tri.norm3 * c.y);
        //chit.uv = tri.uv1 * c.z + tri.uv2 * c.x + tri.uv3 * c.y;
        hit.position = (ray.origin + ray.direction * tt);
        hit.isHit = true;
        ray.tEnd = tt;
        return true;
    }
    return false;
}

bool isect_triPre(inout Ray ray, int index, inout Hit hit ) {
	vec4 normal = texelFetch(texPreInData, 4 * index).rgba;
	float k = dot(normal.xyz, ray.direction);

	if(k == 0.0)
		return false;

	float s = (normal.a - dot(normal.xyz, ray.origin)) / k;
	if(s <= 0.0)
		return false;

	vec3 hitPos = ray.direction*s;
	hitPos += ray.origin;

	vec4 ee1 = texelFetch(texPreInData, 4 * index + 1);
	float kt1 = dot(ee1.xyz, hitPos) - ee1.a; 
	if (kt1 < 0.0f)
	 return false;
	
	vec4 ee2 = texelFetch(texPreInData, 4 * index + 2);
	float kt2 = dot(ee2.xyz, hitPos) - ee2.a; 
	if (kt2 < 0.0f)
	 return false;
	
	vec4 ee3 = texelFetch(texPreInData, 4 * index + 3);
	float kt3 = dot(ee3.xyz, hitPos) - ee3.a; 
	if (kt3 < 0.0f)
	 return false;

	float hitZ = distance(ray.origin, hitPos);

	if (hitZ < ray.tEnd) {
		
		// maintain the closest hit
		ray.tEnd = hitZ;
        hit.normal = normal.xyz;//(tri.norm1 * c.z + tri.norm2 * c.x + tri.norm3 * c.y);
        hit.position = hitPos;
        hit.isHit = true;
        return true;

	}
	return false;
}

void traceCloseHitV2(inout Ray ray, inout Hit hit)
{
    stackClear();
    stackPush(0);
    hit.isHit = false;
    Node select;
    Triangle try;
    float tempt;

    while(stackSize() != 0)
    {
        select = getNode(stackPop());
        if(!slabs(ray, select.aabbMin, select.aabbMax, tempt))
             continue;
        

        if(select.childIsTriangle == 0)
        {
	        // stackPush(select.rightChild);
	        // stackPush(select.leftChild);

	        float leftMinT = 0;
            float rightMinT= 0;
            Node right = getNode(select.rightChild);
            Node left = getNode(select.leftChild);
            bool rightI = slabs(ray, right.aabbMin, right.aabbMax,  rightMinT);
            bool leftI = slabs(ray, left.aabbMin, left.aabbMax,  leftMinT);

            if (rightMinT < leftMinT)
			{
			    stackPush(select.leftChild);
			    stackPush(select.rightChild);
			}
			else
			{
			    stackPush(select.rightChild);
			    stackPush(select.leftChild);
			}
        }
        else
        {
        	for (int i = select.rightChild; i < select.rightChild + select.leftChild; i++)
        	{
        		int idx = texelFetch(texIndex, i).x;
        		//try = getTriangle(idx);
        		isect_triPre(ray, idx, hit);
        		//isect_tri(ray, try, hit);
        	}
        }





        // if(select.childIsTriangle == 0)
        // {
        //     float leftMinT = 0;
        //     float rightMinT= 0;
        //     Node right = getNode(select.rightChild);
        //     Node left = getNode(select.leftChild);
        //     bool rightI = slabs(ray, right.aabbMin, right.aabbMax,  rightMinT);
        //     bool leftI = slabs(ray, left.aabbMin, left.aabbMax,  leftMinT);

        //     if(rightI && leftI)
        //     {
        //         if (rightMinT < leftMinT)
        //         {
        //             stackPush(select.leftChild);
        //             stackPush(select.rightChild);
        //         }
        //         else
        //         {
        //             stackPush(select.rightChild);
        //             stackPush(select.leftChild);
        //         }
        //         continue;
        //     }
        //     if(rightI)
        //         stackPush(select.rightChild);
        //     else
        //         stackPush(select.leftChild);
        //     continue;
        // }

        // if ((select.childIsTriangle & 2) == 0)
        //     stackPush(select.rightChild);

        // if ((select.childIsTriangle & 1) == 0)
        //     stackPush(select.leftChild);

        // if((select.childIsTriangle & 2) > 0)
        // {
        //     try = getTriangle(select.rightChild);
        //     isect_tri(ray, try, hit);
        // }

        // if((select.childIsTriangle & 1) > 0)
        // {
        //     try = getTriangle(select.leftChild);
        //     isect_tri(ray, try, hit);
        // }
    }
}



void main() {
	vec2 view = 2.0 * fragCoord - 1.0;
	view.y *= aspectRation;
    vec3 viewDir = normalize(vec3(view, 1.0));
    vec3 worldDir = viewToWorld * viewDir;

    Ray ray;
    ray.direction = worldDir;
    ray.origin = location;
    ray.tStart = 0.0001;
    ray.tEnd = 10000;

    Hit hit;
    traceCloseHitV2(ray, hit);
    color = vec4(0.5+hit.normal*0.5, 1.0);
}
